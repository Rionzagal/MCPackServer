...
namespace MCPackServer.Pages.RolesModule
{
    public partial class RoleManagementPage
    {
        ...
        {
            var response = await _rolesService.SearchUsersForRole(roleId);
            List<AspNetUserRoles> result = null != response ? response.ToList() : new();
            return result;
        }
        ...
        private async Task<ICollection<AspNetRoleClaims>> ClaimsServerReload(string roleId)
        {
            ...
            var response = await _service.GetForGridAsync<AspNetRoleClaims>(request);
            if (null != response)
            {
